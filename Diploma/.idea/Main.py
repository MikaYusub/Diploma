...
def direct_problem(eps, M, N, u_left, u_right, t, x, q, h):
    ...
    def DiagonalPreparation(eps, tau, q, h, y):
    ...
    for m in range(M):
        a_diag, b_diag, c_diag = DiagonalPreparation(eps, tau, q, h, y[m, :])
        w_1 = TridiagonalMatrixAlgorithm(a_diag, b_diag, c_diag, func(y[m, :], (t[m] + t[m + 1]) / 2, x, q))
        tmp2 = tau * w_1.real
...
def conjucate_problem(eps, M, N, t, x, q, h, u, f_obs):
    ...
    def DiagonalPreparationConjugate(eps, tau, q, h, u):
        a_diag = np.zeros(N - 1, dtype=complex)
        b_diag = np.zeros(N - 1, dtype=complex)
        c_diag = np.zeros(N - 1, dtype=complex)
    ...
    for m in range(M, 0, -1):
        a_diag_conj, b_diag_conj, c_diag_conj = DiagonalPreparationConjugate(eps, tau, q, h, u[m, :])
        w_1 = TridiagonalMatrixAlgorithm(a_diag_conj,
                           b_diag_conj,
                           c_diag_conj,
